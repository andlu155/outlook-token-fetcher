...
| 自动接码 | 可选对接自建临时邮箱 Admin API 拉取验证码；取码前等待 3 秒，降低取到旧码概率 |
| 页面识别 | 登录、备用邮箱绑定/再验证、验证码、代码发送方式（风控选通道）、失效码、通行密钥取消、保持登录、同意、帐户锁定、人机验证提示等 |
| 批处理控制 | 暂停/继续（保留队列）；停止清空队列；**下一账号**强制切到队列下一号 |
| 缓存清理 | 每处理 5 个账号自动清理 Outlook/Microsoft 登录相关缓存 |
| 异常恢复 | SSL/代理等硬网络错误直接跳过当前账号；软白屏有限恢复 + 整任务重跑 1 次；登录过渡页忽略误判白屏 |
| 结果 | 侧栏展示；可按 **执行先后** 或 **状态分组** 排序；复制 / 手动导出（不自动下载本地文件） |
...
以 [`chrome-extension/manifest.json`](chrome-extension/manifest.json) 中的 `version` 为准（当前 **1.23**）。
